...
test.describe('JSONPlaceholder API - Posts Tests', () => {
  test.beforeAll(async () => {
    reporter.info('Initializing API client for JSONPlaceholder');
  ...
  test('@smoke @api Should get all posts', async () => {
    reporter.step('Fetching all posts');
    ...
    reporter.step(
      `Retrieved ${(response.data as unknown[]).length} posts in ${response.responseTime}ms`
    );
  });

  test('@api Should get post by ID', async () => {
    const postId = 1;
    reporter.step(`Fetching post with ID: ${postId}`);
    ...
    reporter.step(`Retrieved post: ${JSON.stringify(response.data, null, 2)}`);
  });

  test('@api Should create new post', async () => {
    reporter.step('Creating new post');
    ...
    reporter.step(`Created post with ID: ${(response.data as { id: number }).id}`);
  });

  test('@api Should update existing post', async () => {
    const postId = 1;
    reporter.step(`Updating post with ID: ${postId}`);
    ...
    reporter.step('Post updated successfully');
  });

  test('@api Should partially update post with PATCH', async () => {
    const postId = 1;
    reporter.step(`Partially updating post with ID: ${postId}`);
    ...
    reporter.step('Post partially updated successfully');
  });

  test('@api Should delete post', async () => {
    const postId = 1;
    reporter.step(`Deleting post with ID: ${postId}`);
    ...
    reporter.step(`Deleted post with ID: ${postId}`);
  });

  test('@api Should handle 404 for non-existent post', async () => {
    const nonExistentId = 999999;
    reporter.step(`Attempting to fetch non-existent post with ID: ${nonExistentId}`);
    ...
    try {
      ...
      reporter.step('Correctly handled non-existent resource');
      expect(error).toBeDefined();
    }
  });

  test('@api @regression Should verify response time performance', async () => {
    reporter.step('Testing API response time');
    ...
    reporter.step(`API response time: ${response.responseTime}ms - Performance good ✓`);
  });
});
...
test.describe('JSONPlaceholder API - Comments Tests', () => {
  ...
  test('@smoke @api Should get comments for a post', async () => {
    const postId = 1;
    reporter.step(`Fetching comments for post ID: ${postId}`);
    ...
    reporter.step(`Retrieved ${(response.data as unknown[]).length} comments for post ${postId}`);
  });

  test('@api Should get all comments', async () => {
    reporter.step('Fetching all comments');
    ...
    reporter.step(`Retrieved ${(response.data as unknown[]).length} total comments`);
  });
});
...
test.describe('JSONPlaceholder API - Users Tests', () => {
  ...
  test('@smoke @api Should get all users', async () => {
    reporter.step('Fetching all users');
    ...
    reporter.step(`Retrieved ${(response.data as unknown[]).length} users`);
  });

  test('@api Should get user by ID with complete details', async () => {
    const userId = 1;
    reporter.step(`Fetching user with ID: ${userId}`);
    ...
    reporter.step(`User details: ${(response.data as any).name} (${(response.data as any).email})`);
  });
});
...
test.describe('API Client Features Demonstration', () => {
  ...
  test('@api Should demonstrate retry mechanism', async () => {
    reporter.step('Testing API retry mechanism');
    ...
    reporter.step('Retry mechanism working correctly');
  });

  test('@api Should handle custom headers', async () => {
    reporter.step('Testing custom headers');
    ...
    reporter.step('Custom headers handled successfully');
  });
});
